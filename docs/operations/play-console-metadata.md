...
| Foreground service / wake lock | Model download and production opt-in voice runtime continuity | Operational only; no universal background-reliability claim |
| Microphone | Dictation, assistant capture, and production opt-in hands-free Offas | Excluded from the frozen listing claims/assets pending retained qualification |
| Camera | Flashlight on/off through Android's torch API when the user requests that voice action | No photo or video capture; optional hardware feature |
| Set alarm | Send validated alarm and timer requests to a compatible Clock app | The Clock app owns the resulting alarm or timer |
| System file picker | User-selected GGUF model and single-image attachment | No broad device-storage access claim |

The app does not request location, contacts, SMS, or advertising-ID access.
...
5. Voice is available as a production opt-in. Do not publish broad wake, battery, background-reliability, or supported-device claims until retained multi-OEM and 24-hour qualification supports them.
