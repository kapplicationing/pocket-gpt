...
| Voice activation and hands-free command path (limited beta only) | keeps voice in scope for controlled cohorts without turning it into a broad public launch promise | local voice-model install and microphone permission are hard blockers; OEM battery policy, assistant-role setup, and privacy wording remain support/follow-up constraints | Implemented (limited-beta path) | Implemented (Advanced-only toggle + readiness/setup guidance + bounded device-action tools); user-visible but not a general launch claim |
