...
11. HF flows must open the model library through `tests/maestro/shared/open-model-library-debug.yaml`, then enter `shared/open-model-library-advanced-sources.yaml`. The first helper launches `com.pocketagent.android.DEBUG_OPEN_MODEL_LIBRARY`, skips onboarding through the real `ChatViewModel` path, and waits for `debug_model_library_ready`; the second follows the explicit Explore → Advanced sources path. Do not use shared onboarding/bootstrap helpers for HF flow setup.
